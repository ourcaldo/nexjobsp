...
interface ArticleFilteredContentProps {
...
/**
 * Client island for the article listing page.
 * Handles: category filter, article grid, pagination, tag filter, sidebar.
 * All state is interconnected (category change resets page & tag, etc.).
 */
export default function ArticleFilteredContent({
  initialArticles,
  categories,
  latestArticles,
  tags,
  initialTag
}: ArticleFilteredContentProps) {
                      ...
                      <div className="p-6">
                        {article.categories && article.categories.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-3">
                            {article.categories.slice(0, 2).map((category: any, catIndex: number) => (
                              <span
                                key={category.id}
